    ...
    <table class="ui compact celled padded striped table checkboxs sortable">
        <thead>
            <tr>
                ...
                <th width="1" class="center aligned">İkon</th>
                <th>Başlık</th>
                <th width="300" class="center aligned">Renk</th>
                <th width="1" class="center aligned">Sıra</th>
        ...
        <tbody id="table-{!! $appRouter['current']['as'] !!}">
            @foreach($rows as $row)
                <tr id="row-{!! $appRouter['current']['as'] !!}-{!! $row['id'] !!}">
                    ...
                    <td class="center aligned">
                        <span class="ui label" style="background:{!! $row['bg_color'] !!}; color: {!! $row['text_color'] !!}">
                            {!! $row['text_color'] !!}, {!! $row['bg_color'] !!}
                        </span>
                    </td>
                    <td class="center aligned">
                        <div class="ui label orange">{!! $row['row'] !!}</div>
                    </td>
